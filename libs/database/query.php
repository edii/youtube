...
class InsertQuery extends Query {
  ...
  public function preExecute() {
    ...
    // exception.
    if (!isset($this->insertValues[0]) and count($this->insertFields) > 0 and empty($this->fromQuery)) {
      return FALSE;
    }
    return TRUE;
  }
...
class DatabaseCondition implements QueryConditionInterface, Countable {
  ...
  public function compile(DatabaseConnection $connection, QueryPlaceholderInterface $queryPlaceholder) {
    ...
    if ($this->changed || isset($this->queryPlaceholderIdentifier) and ($this->queryPlaceholderIdentifier != $queryPlaceholder->uniqueIdentifier())) {
