 ...
 */
namespace core\imagetoolkit\geometry;
...
class Rectangle {
	...
	public function __construct() {
		...
		if (count($args) == 4) {
			...
			and $args[0] instanceof Point
			and $args[1] instanceof Size) {
	...
	public function isIntersectsWith(Rectangle $rect) {
		return $this->isIntersectsWithX($rect) and $this->isIntersectsWithY($rect);
	}
	...
	public function center() {
		...
		if (count($args) == 4) {
			return $this->center(\Rectangle($args[0], $args[1], $args[2], $args[3]));
		} else if (count($args) == 1 and $args[0] instanceof Rectangle) {
			$rect = $args[0];
